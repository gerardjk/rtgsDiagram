...
function initializeDiagram() {
  ...
  ensureVisibilityStyles();
    ...
    // Function to skip animation and reveal everything immediately
    const skipAnimation = () => {
      ...
      if (window.stopTypewriter) {
        window.stopTypewriter();
      }

      // Complete divider animation immediately
      if (window.dividerElements) {
        ...
        if (window.dividerElements.upLabel) {
          ...
          const fullText = window.dividerElements.upLabel.dataset.fullText || 'Low Value / High Volume';
          window.dividerElements.upLabel.textContent = fullText;
        }
        if (window.dividerElements.upArrow) {
          window.dividerElements.upArrow.style.opacity = '1';
        }
        if (window.dividerElements.downLabel) {
          window.dividerElements.downLabel.style.opacity = '1';
          const fullDownText = window.dividerElements.downLabel.dataset.fullText || 'High Value / Low Volume';
          window.dividerElements.downLabel.textContent = fullDownText;
        }
        if (window.dividerElements.downArrow) {
          window.dividerElements.downArrow.style.opacity = '1';
        }
      }
    ...
    for (let i = 0; i < numCircles; i++) {
      ...
      if (i === 0) {
        ...
        blackCircle.setAttribute('id', 'rba-black-circle');
        blackCircle.classList.add('diagram-hidden'); // Hide initially for animation
      ...
      if (i === 99) {
        ...
        if (i === 0) {
          circle.setAttribute('id', 'rba-blue-dot');
          circle.classList.add('diagram-hidden'); // Hide initially for animation
          const bigGroup = document.getElementById('big-group');
          if (bigGroup && bigGroup.parentNode) {
      ...
      if (i === 0) {
        ...
        text.setAttribute('id', 'rba-label');
        text.classList.add('diagram-hidden'); // Hide initially for animation
        labelsGroup.appendChild(text);
        console.warn('Created RBA label at position:', actualCircleX + blackCircleRadius + 5, actualCircleY);
        ...
        // Make CHESS-RTGS box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(narrowRect, 'chess-rtgs-box');
          makeInteractive(chessText, 'chess-rtgs-box');
        }
        ...
        // Make Austraclear box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(austraclearRect, 'austraclear-box');
          makeInteractive(austraclearText, 'austraclear-box');
        }
        ...
        // Make DvP RTGS box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(dvpRtgsRect, 'dvp-rtgs-box');
          makeInteractive(dvpRtgsText, 'dvp-rtgs-box');
        }
        ...
        // Make ASXB box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(bridgeBox, 'asxb-box');
          makeInteractive(asxSettlementText, 'asxb-box');
        }
        ...
        // Make ASXF box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(asxfBox, 'asxf-box');
          makeInteractive(asxfText, 'asxf-box');
        }
        ...
        // Make RTGS box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(rtgsRect, 'rtgs-box');
          makeInteractive(rtgsText, 'rtgs-box');
        }
        ...
        // Make PEXA box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(pexaBox, 'pexa-box');
          makeInteractive(pexaText, 'pexa-box');
        }
        ...
        // Make ESSB box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(eftposBox, 'essb-box');
          makeInteractive(eftposText, 'essb-box');
        }
        ...
        // Make MCAU box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(mastercardBox, 'mcau-box');
          makeInteractive(mastercardText, 'mcau-box');
        }
        ...
        // Make NPP BI box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(nppBox, 'npp-box');
          makeInteractive(nppTopText, 'npp-box');
          makeInteractive(nppBottomText, 'npp-box');
        }
        ...
        // LVSS lines will be added after LVSS circle is created
          ...
          // Make Administered Batches box interactive
          if (typeof makeInteractive === 'function') {
            makeInteractive(adminBatchesBox, 'administered-batches-box');
            makeInteractive(adminBatchesText, 'administered-batches-box');
          }
        ...
        // Make cash transfer box interactive
        if (typeof makeInteractive === 'function') {
        ...
        if (window.swiftHvcsElements && window.swiftHvcsElements.boundingBox) {
          ...
          if (swiftPdsRect) {
            ...
            if (Math.abs(horizontalGap - verticalGap) < 0.01) {
              ...
              const hvcsStrokeHalfCorrected = window.hvcsLineStrokeHalf || 3;
              const correctedPath = window.swiftHvcsElements.hvcsLine.getAttribute('d');
              const correctedNewPath = correctedPath.replace(/M ([\d.]+)/, `M ${(correctedHvcsBoxRight + hvcsStrokeHalfCorrected).toFixed(2)}`);
              window.swiftHvcsElements.hvcsLine.setAttribute('d', correctedNewPath);

              if (window.hvcsLineData) {
                window.hvcsLineData.startX = correctedHvcsBoxRight + hvcsStrokeHalfCorrected;
              }
        ...
        // Make CHESS box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(chessEquitiesRect, 'chess-box');
          makeInteractive(chessEquitiesText, 'chess-box');
        }
        ...
        // Make Sympli e-conveyancing box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(sympliBox, 'sympli-box');
          makeInteractive(sympliText, 'sympli-box');
        }
        ...
        // Make PEXA e-conveyancing box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(pexaConveyBox, 'pexa-convey-box');
          makeInteractive(pexaConveyText, 'pexa-convey-box');
        }
        ...
        if (window.hexagonPositions) {
          ...
          const createStackedRect = (y, fillColor, label, xOverride, heightOverride, fontSizeOverride, id) => {
            ...
            const text = createStyledText(
              ...
              label,
              {
                ...
                fontWeight: (label === 'eftpos' || label === 'Mastercard' || isNarrowBox) ? 'bold' : 'normal' // Bold text for eftpos, Mastercard, and narrow boxes
              }
            );
            labelsGroup.appendChild(text);
          ...
          const medicare = createStackedRect(medicareY, '#32cd32', 'Claims', eftposActualX, reducedHeight, reducedFont, 'claims-box');
          const atmsY = medicareY - gapHalf - reducedHeight;
          const atms = createStackedRect(atmsY, '#C08552', 'ATMs', eftposActualX, reducedHeight, reducedFont, 'atms-box');

          // Make the IAC inner boxes interactive
          if (typeof makeInteractive === 'function') {
            makeInteractive(atms.rect, 'atms-box');
            makeInteractive(atms.text, 'atms-box');
            makeInteractive(medicare.rect, 'claims-box');
            makeInteractive(medicare.text, 'claims-box');
            makeInteractive(visa.rect, 'other-cards-box');
            makeInteractive(visa.text, 'other-cards-box');
            makeInteractive(otherCards.rect, 'visa-box');
            makeInteractive(otherCards.text, 'visa-box');
            makeInteractive(eftpos.rect, 'eftpos-box');
            makeInteractive(eftpos.text, 'eftpos-box');
            makeInteractive(mastercard.rect, 'mastercard-box');
            makeInteractive(mastercard.text, 'mastercard-box');
          }

          // Store data to draw white dot later after eftpos line is created
          window.whiteDotData = {
            eftposBottom: eftposY + stackedHeight,
            pexaConveyTop: pexaConveyY,
          ...
          const stackLineConfigs = [
            ...
            },
            {
              ...
              color: '#32cd32', // Match Claims box green
              fraction: 0.6,
              offset: 16,
              strokeWidth: 1.5,
              lineCap: 'butt',
          ...
          const stackLines = stackLineConfigs.map((config) => {
            ...
            // Add identifier for highlighting (matches what hit area triggers)
            path.setAttribute('data-interactive-id', config.id);
            // Visible line doesn't need pointer events - hit area handles it
          ...
          window.updateDirectEntryBoundingLine = (leftEdge, top, height) => {
              ...
              if (window.adiBoxData && Number.isFinite(window.adiBoxData.y)) {
                ...
                if (geometry) {
                  ...
                  if (Number.isFinite(geometry.control2X) && Number.isFinite(geometry.extendPastReference)) {
                    ...
                    else if (color === '#32cd32') controlOffset = 8; // Green (Claims box color)
                  ...
                  if (Number.isFinite(geometry.endX)) {
                    ...
                    else if (color === '#32cd32') endpointOffset = 12; // Green - 2px right of brown (Claims box color)
            ...
            lines.forEach((entry) => {
              ...
              if (!entry.horizontalBranch) {
                ...
                // Add identifier for highlighting (matches what hit area triggers)
                horizontalBranch.setAttribute('data-interactive-id', entry.id);
                ...
                // Create hit area for horizontal branch
                const horizontalHitArea = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                horizontalHitArea.setAttribute('stroke', 'transparent');
                horizontalHitArea.setAttribute('stroke-width', '10');
                horizontalHitArea.setAttribute('stroke-linecap', 'round');
                horizontalHitArea.setAttribute('fill', 'none');
                horizontalHitArea.setAttribute('id', `${entry.id}-horizontal-hit-area`);
                horizontalHitArea.style.pointerEvents = 'stroke';
                // Make the hit area interactive
                if (typeof window.makeInteractive === 'function') {
                  window.makeInteractive(horizontalHitArea, entry.id);
                }
                entry.horizontalBranchHitArea = horizontalHitArea;
              }
              ...
              const buildHorizontalPath = () => {
                ...
                if (window.adiBoxData && Number.isFinite(window.adiBoxData.y)) {
                  ...
                  if (window.nonAdiBoxData) {
                    ...
                    else if (entry.color === '#32cd32') nonAdiEntryY += 2; // Green - 3rd from top (Claims box color)
              ...
              if (horizontalPathData && entry.horizontalBranch) {
                ...
                // Also update hit area path
                if (entry.horizontalBranchHitArea) {
                  entry.horizontalBranchHitArea.setAttribute('d', horizontalPathData);
                }
              }
            });

            // After all path updates, insert thin horizontal branches and ensure z-order
            if (backgroundGroup && lines && lines.length > 0) {
              // Insert all pending thin lines first
              lines.forEach((entry) => {
                if (entry.horizontalBranchPendingInsert && entry.horizontalBranch) {
                  backgroundGroup.appendChild(entry.horizontalBranch);
                  // Also insert hit area after visible branch
                  if (entry.horizontalBranchHitArea) {
                    backgroundGroup.appendChild(entry.horizontalBranchHitArea);
                  }
                  entry.horizontalBranchPendingInsert = false;
                }
              });

              // Now reorder: move all thick lines AFTER thin lines in DOM
              // Search entire SVG for thick lines (they may be direct svg children)
              const svgEl = backgroundGroup.ownerSVGElement || document.querySelector('svg');
              if (svgEl) {
                const thickLines = svgEl.querySelectorAll('.thick-line-to-adi');
                thickLines.forEach(thickLine => {
                  // Move to end of backgroundGroup (after thin lines)
                  backgroundGroup.appendChild(thickLine);
                });
              }
            }
          };
          ...
          if (window.hexagonPositions) {
            ...
            if (!eftposRect || !mastercardRect) {
              ...
              // Add identifier for highlighting (matches what hit area triggers)
              eftposUpturnPath.setAttribute('data-interactive-id', 'eftpos-left-line');
              // Visible line doesn't need pointer events - hit area handles it
              ...
              // Add identifier for highlighting (matches what hit area triggers)
              mastercardUpturnPath.setAttribute('data-interactive-id', 'mastercard-left-line');
              // Visible line doesn't need pointer events - hit area handles it
              ...
              const createHorizontalBranch = (segments, color, id) => {
                ...
                if (segments.horizontalY && segments.curveStartX && segments.pathString && window.adiBoxData) {
                  ...
                  if (lastCIndex > -1) {
                    // Add horizontal extension with downward curve, then left curve stopping at non-ADIs border
                ...
                // Insert before thick lines to ADI so thin lines render under them
                const backgroundGroup = document.getElementById('background-elements');
                horizontalPath.classList.add('thin-nonadi-line');
                // Store for later insertion - we'll insert after thick lines are positioned
                horizontalPath.dataset.pendingInsert = 'true';
                return horizontalPath;
              };
              ...
              // Add identifier for highlighting (matches what hit area triggers)
              eftposHorizontalPath.setAttribute('data-interactive-id', 'eftpos-left-line-horizontal');
              // Visible line doesn't need pointer events - hit area handles it
              ...
              eftposHorizontalHitArea.setAttribute('stroke-width', '10');
              ...
              // Add identifier for highlighting (matches what hit area triggers)
              mastercardHorizontalPath.setAttribute('data-interactive-id', 'mastercard-left-line-horizontal');
              // Visible line doesn't need pointer events - hit area handles it
              ...
              mastercardHorizontalHitArea.setAttribute('stroke-width', '10');
          ...
          // White dot will be drawn later after the purple PEXA line is created

          // Insert eftpos/mastercard horizontal paths and ensure z-order
          const bgGroup = document.getElementById('background-elements');
          if (bgGroup) {
            // Insert thin lines
            if (eftposHorizontalPath && eftposHorizontalPath.dataset.pendingInsert === 'true') {
              bgGroup.appendChild(eftposHorizontalPath);
              delete eftposHorizontalPath.dataset.pendingInsert;
            }
            if (mastercardHorizontalPath && mastercardHorizontalPath.dataset.pendingInsert === 'true') {
              bgGroup.appendChild(mastercardHorizontalPath);
              delete mastercardHorizontalPath.dataset.pendingInsert;
            }

            // Move all thick lines AFTER thin lines in DOM so they render on top
            // Search entire SVG for thick lines (they may be direct svg children)
            const svgEl = bgGroup.ownerSVGElement || document.querySelector('svg');
            if (svgEl) {
              const thickLines = svgEl.querySelectorAll('.thick-line-to-adi');
              thickLines.forEach(thickLine => {
                bgGroup.appendChild(thickLine);
              });
            }
          }
          ...
          // Single thick red line from DE to ADIs (visible only, no interactivity)
          const line = document.createElementNS('http://www.w3.org/2000/svg', 'path');
          line.setAttribute('id', 'directentry-to-adi-line');
          line.setAttribute('data-interactive-id', 'directentry-to-adi-line'); // For highlighting
          line.setAttribute('stroke', '#ff073a'); // Red
          line.setAttribute('stroke-width', '4');
          line.setAttribute('stroke-linecap', 'round');
          line.setAttribute('fill', 'none');
          line.style.pointerEvents = 'none'; // No pointer events on visible line
          // Insert at very beginning so line renders under all boxes
          svg.insertBefore(line, svg.firstChild);
          directEntryToAdiLines.push(line);

          // Create transparent hit area for main DE line (interactive, on top)
          const deLineHitArea = document.createElementNS('http://www.w3.org/2000/svg', 'path');
          deLineHitArea.setAttribute('id', 'directentry-to-adi-line-hit-area');
          deLineHitArea.setAttribute('stroke', 'transparent');
          deLineHitArea.setAttribute('stroke-width', '16'); // Larger for easier clicking
          deLineHitArea.setAttribute('stroke-linecap', 'round');
          deLineHitArea.setAttribute('fill', 'none');
          deLineHitArea.style.pointerEvents = 'stroke';
          if (window.makeInteractive) {
            window.makeInteractive(deLineHitArea, 'directentry-to-adi-line');
          }
          // Append to end for top z-order
          svg.appendChild(deLineHitArea);
          window.deLineHitArea = deLineHitArea;

          // Create horizontal branch for red line (visible only, no interactivity)
          const maroonHorizontalBranches = [];
          const horizontalBranch = document.createElementNS('http://www.w3.org/2000/svg', 'path');
          horizontalBranch.setAttribute('id', 'maroon-horizontal-branch');
          horizontalBranch.setAttribute('data-interactive-id', 'directentry-to-adi-line'); // For highlighting (same ID as main line)
          ...
          horizontalBranch.setAttribute('fill', 'none');
          horizontalBranch.style.pointerEvents = 'none'; // No pointer events on visible line
          // Insert at very beginning so line renders under all boxes
          ...
          // Create transparent hit area for horizontal branch (interactive, on top)
          const maroonBranchHitArea = document.createElementNS('http://www.w3.org/2000/svg', 'path');
          maroonBranchHitArea.setAttribute('id', 'maroon-horizontal-branch-hit-area');
          maroonBranchHitArea.setAttribute('stroke', 'transparent');
          maroonBranchHitArea.setAttribute('stroke-width', '12'); // Larger for easier clicking
          maroonBranchHitArea.setAttribute('stroke-linecap', 'round');
          maroonBranchHitArea.setAttribute('fill', 'none');
          maroonBranchHitArea.style.pointerEvents = 'stroke';
          if (typeof window.makeInteractive === 'function') {
            window.makeInteractive(maroonBranchHitArea, 'directentry-to-adi-line');
          }
          // Append to end for top z-order
          svg.appendChild(maroonBranchHitArea);
          window.maroonBranchHitArea = maroonBranchHitArea;
          ...
          const updateDirectEntryToAdiLine = () => {
            ...
            // Create the visible C-curve path directly on the main line
            {
                ...
                // Also update the hit area with same path
                if (window.deLineHitArea) {
                  window.deLineHitArea.setAttribute('d', cCurvePathData);
                }
                ...
                if (window.maroonHorizontalBranches && window.maroonHorizontalBranches[0] && window.adiBoxData) {
                  ...
                  // Also update the hit area with same path
                  if (window.maroonBranchHitArea) {
                    window.maroonBranchHitArea.setAttribute('d', horizontalBranchPath);
                  }
                }
            }
          ...
          oskoToAdiHitArea.setAttribute('stroke-width', '10');
        ...
        // Make trade-by-trade box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(tradeByTradeRect, 'trade-by-trade-box');
          makeInteractive(tradeByTradeText, 'trade-by-trade-box');
        }
        ...
        // Make clearing/netting box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(clearingRect, 'clearing-box');
          makeInteractive(clearingText, 'clearing-box');
        }
        ...
        // Make DvP cash leg box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(sssCcpRect, 'dvp-cash-leg-box');
          makeInteractive(sssCcpText, 'dvp-cash-leg-box');
        }
        ...
        // Make ASX box interactive
        if (typeof makeInteractive === 'function') {
          makeInteractive(asxBoundingBox, 'asx-box');
        }
        ...
        if (pexaConveyBox) {
          ...
          if (window.pexaExtensions) {
            ...
            if (headerInfo && headerInfo.rect) {
              ...
              // Make BPAY box interactive
              if (typeof makeInteractive === 'function') {
                makeInteractive(bpayBox, 'bpay-box');
                makeInteractive(bpayText, 'bpay-box');
              }
        ...
        if (window.asxBoxData && window.asxBoxData.needsLine && window.hvcsLineData) {
          ...
          // Visible line doesn't need pointer events - hit area handles it
          asxToHvcsLineStyled.style.pointerEvents = 'none';

          // Create invisible hit area for easier hovering
          const asxToHvcsHitArea = createStyledPath(asxPath, {
            stroke: 'transparent',
            strokeWidth: '6',
            fill: 'none',
            strokeLinecap: 'butt',
            id: 'asx-to-hvcs-line-hit-area'
          });
          asxToHvcsHitArea.style.pointerEvents = 'stroke';
          if (window.makeInteractive) {
            window.makeInteractive(asxToHvcsHitArea, 'asx-to-hvcs-line');
          }
          ...
          if (window.asxBoundingBoxElement) {
            ...
            window.asxBoundingBoxElement.parentNode.insertBefore(asxToHvcsHitArea, asxToHvcsLineStyled.nextSibling);
          } else {
            svg.insertBefore(asxToHvcsLineStyled, svg.firstChild);
            svg.insertBefore(asxToHvcsHitArea, asxToHvcsLineStyled.nextSibling);
          }
          ...
          // Visible line doesn't need pointer events - hit area handles it
          asxToAdiLineStyled.style.pointerEvents = 'none';

          // Create invisible hit area for easier hovering
          const asxToAdiHitArea = createStyledPath(asxPath2, {
            stroke: 'transparent',
            strokeWidth: '6',
            fill: 'none',
            strokeLinecap: 'butt',
            id: 'asx-to-adi-line-hit-area'
          });
          asxToAdiHitArea.style.pointerEvents = 'stroke';
          if (window.makeInteractive) {
            window.makeInteractive(asxToAdiHitArea, 'asx-to-adi-line');
          }

          // Insert before ASX bounding box so line renders under it
          if (window.asxBoundingBoxElement) {
            window.asxBoundingBoxElement.parentNode.insertBefore(asxToAdiLineStyled, window.asxBoundingBoxElement);
            window.asxBoundingBoxElement.parentNode.insertBefore(asxToAdiHitArea, asxToAdiLineStyled.nextSibling);
          } else {
            svg.insertBefore(asxToAdiLineStyled, svg.firstChild);
            svg.insertBefore(asxToAdiHitArea, asxToAdiLineStyled.nextSibling);
          }
        ...
        curvedLineGroup.classList.add('diagram-hidden'); // Hide initially for animation
  ...
  if (window.dotPositions && window.dotPositions[52] && window.dotPositions[55]) {
      ...
      console.warn('Checking for OPA box creation: dotPositions exists?', !!window.dotPositions, 'dotPositions[0] exists?', !!(window.dotPositions && window.dotPositions[0]));
      if (window.dotPositions && window.dotPositions[0]) {
        console.warn('Creating OPA box with RBA position:', window.dotPositions[0]);
        ...
        opaSquare.setAttribute('id', 'opa-box');
        opaSquare.classList.add('diagram-hidden'); // Hide initially for animation
        labelsGroup.appendChild(opaSquare);
        console.warn('Created OPA box at position:', opaX - opaBoxSize/2, opaY - opaBoxSize/2);
        ...
        opaLabel.setAttribute('id', 'opa-label');
        opaLabel.classList.add('diagram-hidden'); // Hide initially for animation
        ...
        opaLine.setAttribute('id', 'opa-to-rba-line');
        opaLine.classList.add('diagram-hidden'); // Hide initially for animation
  ...
  const drawCurvedDoubleLine = (startX, startY, strokeColor, targetGroup, lineId) => {
    if (!targetGroup) {
      return;
    }
    ...
    if (nearlyHorizontal) {
      ...
      if (lineId) {
        const hitAreaWidth = isPinkColor ? offsetMagnitude * 2 + 10 : offsetMagnitude * 2 + 8;
    ...
    if (lineId) {
      const hitAreaWidth = isPinkColor ? offsetMagnitude * 3 + 12 : offsetMagnitude * 2 + 10;
      ...
      drawCurvedDoubleLine(pexaStartX, pexaStartY, redColor, redLinesGroup, 'pexa-to-rits-line');
        ...
        drawCurvedDoubleLine(asxfStartX, asxfStartY, redColor, redLinesGroup, 'asxf-to-rits-line');
        ...
        drawCurvedDoubleLine(mcauStartX, mcauStartY, yellowColor, yellowLinesGroup, 'mcau-to-rits-line');
        ...
        drawCurvedDoubleLine(essbStartX, essbStartY, yellowColor, yellowLinesGroup, 'essb-to-rits-line');
        ...
        drawCurvedDoubleLine(asxbStartX, asxbStartY, yellowColor, yellowLinesGroup, 'asxb-to-rits-line');
      }
      ...
      if (minX9 !== null && maxX9 !== null) {
        ...
        if (minX2 === null || maxX2 === null || minY2 === null || maxY2 === null) {
          ...
          if (window.sympliBoxData && window.adiBoxData && window.asxLineData) {
            ...
            // Add identifier for highlighting (matches what hit area triggers)
            sympliToAdiLineStyled.setAttribute('data-interactive-id', 'sympli-to-adis-line');
            // Visible line doesn't need pointer events - hit area handles it
          ...
          if (window.pexaConveyBoxData && window.adiBoxData && window.asxLineData) {
            ...
            window.purpleLineVerticalX = controlX; // Store for title alignment
            ...
            // Add identifier for highlighting (matches what hit area triggers)
            pexaToAdiLineStyled.setAttribute('data-interactive-id', 'pexa-to-adis-line');
            // Visible line doesn't need pointer events - hit area handles it
            ...
            if (window.whiteDotData && window.pexaConveyBoxData) {
              ...
              // Extend line to the right, fading to black by the left edge of PEXA e-conveyancing box
              const lineEndX = window.pexaConveyBoxData.x; // Fade reaches black at PEXA box left edge

              // Create gradient for fade effect using userSpaceOnUse
              const svg = document.getElementById('diagram');
              const defs = svg.querySelector('defs') || svg.insertBefore(document.createElementNS('http://www.w3.org/2000/svg', 'defs'), svg.firstChild);
              const gradient = document.createElementNS('http://www.w3.org/2000/svg', 'linearGradient');
              gradient.setAttribute('id', 'white-fade-gradient');
              gradient.setAttribute('gradientUnits', 'userSpaceOnUse');
              gradient.setAttribute('x1', lineStartX);
              gradient.setAttribute('x2', lineEndX);
              gradient.setAttribute('y1', lineY);
              gradient.setAttribute('y2', lineY);
              ...
              const fadeStartPercent = ((originalLineEndX - lineStartX) / (lineEndX - lineStartX)) * 100;
          ...
          if (hvcsHorizontalLine && window.hvcsLineData && window.adiBoxData) {
            ...
            if (window.swiftHvcsElements && window.swiftHvcsElements.boundingBox) {
              ...
              // Start path at visual right edge of box
              hvcsStartX = boxX + boxWidth + (boxStroke / 2);
            }
        ...
        if (window.asxLine2Data && window.adiBoxData) {
          ...
          if (asxLine2Extension) {
            ...
            // Also update the hit area with the extended path
            const asxToAdiHitArea = document.getElementById('asx-to-adi-line-hit-area');
            if (asxToAdiHitArea) {
              asxToAdiHitArea.setAttribute('d', extensionPath);
            }
            // Line stays in original position (before ASX box) - don't move it
          }
        }
        ...
        nonADIsText.classList.add('diagram-hidden'); // Hide initially for animation
        labelsGroup.appendChild(nonADIsText);
        console.warn('Created Non-ADIs label');
        if (typeof makeInteractive === 'function') {
        ...
        if (window.asxLineData && window.nonAdiBoxData) {
          ...
          if (asxExtension) {
            ...
            // Also update the hit area with the extended path
            const asxToHvcsHitArea = document.getElementById('asx-to-hvcs-line-hit-area');
            if (asxToHvcsHitArea) {
              asxToHvcsHitArea.setAttribute('d', extensionPath);
            }
          }
        }
...
// Insert into labelsGroup so it's above the osko-to-adi hit area (which intercepts hover events)
// The internal boxes (PayID, PayTo, BSCT) are created later and will still be on top
labelsGroup.appendChild(newBoundingBox);

// Make purple NPP box interactive
if (typeof makeInteractive === 'function') {
  makeInteractive(newBoundingBox, 'npp-purple-box');
}
...
// Make NPP label interactive (same ID as purple box)
if (typeof makeInteractive === 'function') {
  makeInteractive(nppBoundingLabel, 'npp-purple-box');
}

const newPacsBox = createStyledRect(pacsBoxX, pacsBoxY, pacsBoxWidth, pacsBoxHeight, {
  ...pacsStyle,
  id: 'bsct-box'
});
...
// Make BSCT box interactive
if (typeof makeInteractive === 'function') {
  makeInteractive(newPacsBox, 'bsct-box');
  makeInteractive(newPacsText, 'bsct-box');
}
...
const payToBox = createStyledRect(pacsBoxX, adjustedPayToBoxY, pacsBoxWidth, pacsBoxHeight, {
  ...purpleBoxStyle,
  id: 'payto-box'
});
...
// Make PayTo box interactive
if (typeof makeInteractive === 'function') {
  makeInteractive(payToBox, 'payto-box');
  makeInteractive(payToText, 'payto-box');
}
...
const payIdBox = createStyledRect(pacsBoxX, adjustedPayIdBoxY, pacsBoxWidth, pacsBoxHeight, {
  ...purpleBoxStyle,
  id: 'payid-box'
});
...
// Make PayID box interactive
if (typeof makeInteractive === 'function') {
  makeInteractive(payIdBox, 'payid-box');
...
// Make Osko box interactive
if (typeof makeInteractive === 'function') {
  makeInteractive(oskoBox, 'osko-box');
  makeInteractive(oskoText, 'osko-box');
}
...
// Create S-curve from CLS AUD line to CLS dot at the very end
  ...
  if (window.clsEndpoints &&
    ...
    if (Number.isFinite(startX) && Number.isFinite(startY) && Number.isFinite(endX) && Number.isFinite(endY)) {
      ...
      if (!container && svg) {
        ...
        container.classList.add('diagram-hidden'); // Hide initially for animation
        const blueCirclesGroup = document.getElementById('blue-circles');
  ...
  // Allow skipping entire animation with click or any key press
  const skipAnimationHandler = () => {
    // Use the global skipAnimation function which skips everything
    if (typeof window.skipAnimation === 'function') {
      window.skipAnimation();
    }
  };
  ...
  setTimeout(() => {
    ...
    // DE lines: visible lines stay at bottom (inserted first), hit areas at top (appended last)
    // No repositioning needed - z-order is correct
